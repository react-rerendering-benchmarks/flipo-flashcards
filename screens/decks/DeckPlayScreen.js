...
const DeckPlayScreen = ({
  ...
  return <View className='bg-primary dark:bg-primary-dark'>
          <View className="flex-rows items-center h-full">
            ...
            {/* Card flip button */}
            <FlipoButton className={`my-10 px-16 ${flipButton ? '' : 'hidden'}`} onPress={useCallback(() => flipCard(), [])}>
                <FlipoText weight='black' className='text-2xl text-primary tracking-wide'>
                  Flip
                </FlipoText>
            </FlipoButton>
            ...
            <View className={`w-full grow justify-end ${rateButtons ? '' : 'hidden'}`}>
              ...
              <View className={'flex-row w-full justify-center'}>
                {rateButons.map(button => <RateButton key={button} onPress={useCallback(() => rateRecall(flashcard, button), [flashcard])}>{button}</RateButton>)}
              </View>
            </View>
          </View>
      </View>;
