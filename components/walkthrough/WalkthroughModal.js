...
const WalkthroughModal = memo(({
  i18n,
  visible,
  storeShowWalkthrough
}) => {
  i18n.translations = locales;
  const steps = [<View className='space-y-4'>
        <FlipoText className="text-lg text-center text-primary-light dark:text-primary-dark">
  ...
  const stepTitles = ['welcomeTitle', 'sectionsTitle', 'decksSectionTitle', 'profileSectionTitle', 'settingsSectionTitle', 'deckCreationTitle', 'deckPlaythroughTitle', 'deckPlaythroughTitle'];
  ...
  return <FlipoModal title={i18n.t(currentTitle)} visible={open} buttonText={i18n.t('continue')} cancelButton cancelButtonText={i18n.t('skip')} onCancelPress={useCallback(() => {
    storeShowWalkthrough(false);
    setOpen(false);
  }, [setOpen])} onButtonPress={useCallback(() => {
  ...
  }, [setOpen, setCurrentTitle, setCurrentStep, currentStepNumber, setCurrentStepNumber])}>
      {currentStep}
    </FlipoModal>;
});
